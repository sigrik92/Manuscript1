...
# adapted from Jacob Agerbro Rasmussen (https://github.com/JacobAgerbo)
...
pdf("QC_profile_dec24.pdf", height = 15, width = 15)
...
filtered_out = filterAndTrim(fnFs, filtFs, fnRs, filtRs,
                    truncLen=c(280,210), #decide on numbers based on Quality Profile
                    trimLeft = c(17,21), #Remove primers if they are not already removed
                    maxN=0,
                    maxEE=c(2,5),
                    truncQ=2,
                    rm.phix=TRUE,
...
saveRDS(filtered_out, "filtered_out_dec24.RData")

pdf("QC_profile_filtered_dec24.pdf", height = 15, width = 15)
...
pdf("plotErrors_dec24.pdf", height = 15, width = 15)
...
#Inferring of ASVs
#It does this by incorporating the consensus quality profiles and abundances of each unique sequence, and then figuring out if each sequence more likely to be of biological origin or$
...
#maxMismatch is by default 0 but you can make it say 1 or 2 if you ^`^yre finding that a lot of your forward and reverse reads are not merging
...
saveRDS(merged_amplicons, "merged_amplicons_dec24.RData")
...
saveRDS(seqtab.nochim, "seqtab_dec24.RData")
...
write.table(summary_tab, "summary_reads_table_dec24.txt", sep="\t", quote=F)
...
write(asv_fasta, "ASVs_dec24.fa")

#count table:
asv_tab <- t(seqtab.nochim)
write.table(asv_tab, "ASVs_counts_dec24.csv", sep=",", quote=F, col.names=NA)

#tax table:
asv_tax <- taxa
write.table(asv_tax, "ASVs_taxonomy_dec24.csv", sep=",", quote=F, col.names=NA)
